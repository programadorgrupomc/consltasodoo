<template>
  <div class="container">
    <h4>CONSULTAR CONTACTO EMPRESA TIPO PROVEEDOR</h4>
    <form>
            ...
            <div class="modal-content">
              <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">
                  Lista de Contactos: ({{ empresascount }})
                </h1>
                <button
                  type="button"
                  class="btn-close"
      ...
      <div class="mb-3">
        ...
        <p id="advertencia" hidden>
          *Debe Ingresar por lo menos 11 digitos para realizar la busqueda...*
        </p>
      </div>
      ...
      <div class="mb-3">
        <div class="label">
          <label id="label1" class="form-label" hidden>Razon Social:</label>
        </div>
        <input
          v-model="razonsocial"
          type="text"
      ...
      <div class="mb-3">
        <div class="label">
          <label id="label2" class="form-label" hidden>Direccion:</label>
        </div>
        <input
          v-model="direccion"
          type="text"
      ...
      <div class="mb-3">
        <div class="label">
          <label id="label3" class="form-label" hidden>Condicion:</label>
        </div>
        <input
          v-model="condicion"
          type="text"
...
<script>
function consultarRuc(dni) {
  const url = `https://dniruc.apisperu.com/api/v1/ruc/${dni}?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJlbWFpbCI6ImJlcm51aWplc3VzMzYzQGdtYWlsLmNvbSJ9.VxWlJpfwF3s-bsk2S9-dYWkX8_RmadNiXfK11-nUgFM`;
  ...
  return fetch(url)
    ...
    .catch((error) => {
      console.log("Hubo un error al consultar el Ruc:", error);
    });
}
function Habilitar() {
  const rahab = document.getElementById("razonsocial");
  const dirhab = document.getElementById("direccion");
  const cohab = document.getElementById("condicion");

    rahab.removeAttribute("hidden");
    dirhab.removeAttribute("hidden");
    cohab.removeAttribute("hidden");
...
function desHabilitar() {
  ...
  const cohab = document.getElementById("condicion");
    ...
    rahab.setAttribute("hidden", "true");
    dirhab.setAttribute("hidden", "true");
    cohab.setAttribute("hidden", "true");
...
export default {
  ...
  watch: {
    Empresas: {
      handler: function () {
        this.empresascount = this.Empresas.length;
      },
      deep: true,
    },
  },
  methods: {
    handleInput(event) {
      var rucBus = event.target.value;
      if (rucBus.length < 11) {
        Habilitaradvert();
      } else if (rucBus.length === 11) {
        desHabilitaradvert();
        this.intspiner = true;
        consultarRuc(rucBus).then((data) => {
          setTimeout(() => {
            if (data.success == false) {
              alert("Ruc no encontrado o Ruc Invalido 'Intente Nuevamente'");
              this.Limpiar();
            } else {
              Habilitar();
    ...
    Agregar() {
      ...
      const razs = document.getElementById("razonsocial").value;
    ...
    exportToExcel() {
      ...
      XLSX.writeFile(wb, `Contactos-Empresas-Proveedores${now}.xlsx`);
